...
% ======================================================================
% ACT 5.5: LLM LIST SHUFFLE TEST — Devesh Thavalam Recommendation \#3
% ======================================================================

% ===== SLIDE: DEVESH'S SUGGESTION =====
\begin{frame}{Devesh Thavalam: ``Apply the Shuffle Test to the LLM List''}
  \vspace{4pt}
  \begin{block}{The argument}
    \centering\small
    The human list is 86\% pro-cyclical --- shuffling labels within a meeting barely changes the score distribution.\\
    The LLM list is 52\%/42\% pro-/counter-cyclical --- more balanced, so shuffling should be \textbf{more disruptive IF labels matter}.
  \end{block}
  \vspace{8pt}
  \begin{center}
    \large\textbf{Does the shuffle test produce the same answer with the LLM list?}\\[10pt]
    \normalsize We replicate the identical procedure on the LLM-generated 595 concepts.
  \end{center}
  \vspace{8pt}
  \begin{block}{What we test}
    \centering\small
    (1) Shuffle LLM sentiment columns within each meeting. \quad
    (2) Recompute PC1. \quad
    (3) Re-run Ridge (Forecasts + PC1).\\[2pt]
    100 independent permutations. \quad Compared against the same test on the human 296.
  \end{block}
\end{frame}

% ===== SLIDE: LLM LIST COMPOSITION =====
\begin{frame}{The LLM List Has More Within-Meeting Variation --- But Is 99\% Sparse}
  \vspace{-6pt}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Composition comparison}
        \centering\scriptsize
        \begin{tabular}{lcc}
          \toprule
          & \textbf{Human} & \textbf{LLM} \\
          \midrule
          Concepts            & 296    & 595 \\
          Non-zeros/meeting   & 296.0  & \textbf{5.3} \\
          \% Positive (mean)  & 59.4\% & 56.0\% \\
          \% Positive (std)   & 0.099  & \textbf{0.155} \\
          \% Positive (min)   & 26.7\% & 14.3\% \\
          \% Positive (max)   & 79.7\% & 100.0\% \\
          \bottomrule
        \end{tabular}
      \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
      \begin{center}
        \includegraphics[width=0.90\textwidth]{figures/check4a_composition.png}
      \end{center}
      \vspace{-6pt}
      {\scriptsize LLM list (right): higher variance, wider range, long left tail.}
    \end{column}
  \end{columns}
  \vspace{2pt}
  \begin{alertblock}{Critical caveat}
    \centering\footnotesize
    The LLM list is \textbf{99\% sparse} --- only $\sim$5 concepts receive non-zero scores per meeting.\\
    This means the shuffle test only permutes $\sim$5 labels per meeting, not 595.\\[1pt]
    {\scriptsize\color{gray} The sparsity is by design (LLM only scores concepts the document actually discusses), but it limits permutation-based inference.}
  \end{alertblock}
\end{frame}

% ===== SLIDE: PC1 SHUFFLE TEST =====
\begin{frame}{PC1 Shuffle Test: Labels Don't Matter for Either List}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[width=0.65\textwidth]{figures/check4c_shuffle_pc1.png}
  \end{center}
  \vspace{-8pt}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Human (296)}
        \centering\footnotesize
        Actual R$^2$ = 0.458\\
        Shuffle mean = 0.479 \textbf{(higher!)}\\
        $p = 1.000$
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
  ...
  \vspace{-2pt}
  \begin{center}
    \scriptsize\textbf{Neither list shows label-dependence.} The human list actually fits \textit{better} with shuffled labels.\\[1pt]
    {\scriptsize\color{gray} LLM PC1 is weaker overall ($\Delta$R$^2$ = +0.032 vs.\ +0.079 for dict) due to extreme sparsity.}
  \end{center}
\end{frame}

% ===== SLIDE: FULL MODEL SHUFFLE TEST =====
\begin{frame}{Full-Column Shuffle: Using All Sentiment Columns Directly}
  \vspace{-4pt}
  \begin{block}{Procedure}
    \centering\small
    Skip PC1 reduction. Shuffle columns within each meeting. Re-run Ridge directly on\\[2pt]
    \texttt{np.column\_stack([X\_forecasts, X\_shuffled\_sentiments])}\\[2pt]
    100 permutations. \quad No lags, no squares --- pure concept-level signal.
  \end{block}
  \vspace{4pt}
  \begin{center}
    \footnotesize
    \begin{tabular}{lcccc}
      \toprule
      & \textbf{Actual R$^2$} & \textbf{Shuffle Mean} & \textbf{Std} & \textbf{$p$} \\
      \midrule
      Human (296 cols)  & 0.656 & 0.692 & 0.045 & 0.624 \\
      LLM (595 cols)    & 0.609 & 0.626 & \textbf{0.0003} & 1.000 \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{4pt}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering\small Human: modest increase in R$^2$, meaningful variation across shuffles.
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering\small LLM: near-zero variation --- shuffling 5 non-zero scores across 595 columns changes almost nothing.
    \end{column}
  \end{columns}
\end{frame}

% ===== SLIDE: SHOCK CORRELATION =====
\begin{frame}{Shock Correlation: Both Lists Invariant to Label Permutation}
  \vspace{-6pt}
  \begin{center}
    \includegraphics[width=0.62\textwidth]{figures/check4e_shock_corr.png}
  \end{center}
  \vspace{-8pt}
  \begin{center}
    \scriptsize
    \begin{tabular}{lccc}
      \toprule
      & \textbf{Mean $r$} & \textbf{Std} & \textbf{Range} \\
      \midrule
      Human (296)  & 0.9954 & 0.0024 & $[0.9920, 0.9976]$ \\
      LLM (595)    & \textbf{0.9982} & \textbf{0.0012} & $[0.9952, 0.9996]$ \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{4pt}
  \begin{block}{}
    \centering\footnotesize
    \textbf{Both lists produce shocks essentially identical to the original after shuffling.}\\
    The LLM list is \textit{even more} invariant because extreme sparsity means shuffling barely alters the data matrix.\\[1pt]
    {\scriptsize\color{gray} The theoretical ideal (balanced, dense concept list) is not what the LLM produced --- making this a conservative test.}
  \end{block}
\end{frame}

% ===== SLIDE: INTERPRETATION =====
\begin{frame}{Interpretation: Coverage, Not Labels}
  \vspace{2pt}
  \begin{block}{Devesh's premise was correct in principle}
    \centering\footnotesize
    The LLM list has more balanced composition $\to$ shuffle test should be more informative.\\[1pt]
    The data confirms: LLM list has higher within-meeting score variance (std 0.156 vs.\ 0.099), wider range.
  \end{block}
  \vspace{4pt}
  \begin{block}{But the LLM list is also 99\% sparse --- a design feature, not a bug}
    \centering\footnotesize
    The LLM only scores concepts the document actually discusses. Zeros mean ``not mentioned.''\\[1pt]
    This sparsity means only $\sim$5 labels are permuted per meeting --- the shuffle test is underpowered.
  \end{block}
  \vspace{4pt}
  \begin{block}{Yet the answer is unambiguous}
    \centering\footnotesize
    \textbf{Neither list shows label-dependence.} Shock correlations $>$ 0.99 for both.\\[1pt]
    The human list's shuffle R$^2$ actually \textit{increases} --- the original labels may add noise, not signal.\\[1pt]
    \textbf{Labels are incidental. The number of sampling points is what matters.}
  \end{block}
\end{frame}
...
\begin{frame}{Concept Selection Matters --- At the Level of List Composition}
  ...
  \end{block}
  \vspace{6pt}
  \begin{block}{\textbf{New: LLM list shuffle test (Devesh Thavalam Rec.\ \#3)}}
    \centering\small
    Replicated the shuffle test on a 595-concept LLM-generated list with more balanced composition.\\
    \textbf{Same result:} shock correlations $>$ 0.99. Neither list shows label-dependence.\\
    LLM list is 99\% sparse --- a conservative test, but the conclusion holds. \textbf{Coverage, not labels.}
  \end{block}
\end{frame}
